...
<div class="container-fluid">
	...
	<div class="card shadow mb-4">
		...
		<div class="card-body">
			<div class="row"><div class="col-md-5 col-lg-4"><div class="input-group mb-3"><input type="text" class="form-control" id="cariAdmin" placeholder="Cari nama, username, atau email"><div class="input-group-append"><span class="input-group-text bg-primary text-white"><i class="fas fa-search"></i></span></div></div></div></div>
			<div class="table-responsive"><table class="table table-bordered table-hover" width="100%"><thead><tr><th style="width:60px">No</th><th>Admin</th><th>Username</th><th>Email</th><th style="width:105px">Role</th><th style="width:90px">Status</th><th style="width:110px">Aksi</th></tr></thead><tbody id="dataAdmin"><tr><td colspan="7" class="text-center">Memuat data...</td></tr></tbody></table></div>
...
document.addEventListener('DOMContentLoaded', function () {
	...
	function tampilkanAdmin(){var keyword=$('#cariAdmin').val().toLowerCase(),rows=semuaAdmin.filter(function(item){return [item.nama,item.username,item.email,item.role].join(' ').toLowerCase().indexOf(keyword)!==-1;}),html='';if(!rows.length)html='<tr><td colspan="7" class="text-center">Belum ada data admin.</td></tr>';rows.forEach(function(item,index){var foto=item.foto?'<?php echo base_url(); ?>'+item.foto:'<?php echo $admin_asset_path; ?>/img/undraw_profile.svg';html+='<tr class="baris-admin"><td>'+(index+1)+'</td><td><div class="d-flex align-items-center"><img src="'+escapeHtml(foto)+'" alt="" class="rounded-circle mr-2" style="width:42px;height:42px;object-fit:cover"><strong>'+escapeHtml(item.nama)+'</strong></div></td><td>'+escapeHtml(item.username)+'</td><td>'+escapeHtml(item.email)+'</td><td><span class="badge badge-'+(item.role==='superadmin'?'primary':'info')+'">'+escapeHtml(item.role)+'</span></td><td><span class="badge badge-'+(item.status==='aktif'?'success':'secondary')+'">'+escapeHtml(item.status)+'</span></td><td><div class="admin-action-group"><a href="<?php echo site_url('admin/admin_user/edit'); ?>/'+item.id_admin+'" class="btn btn-sm btn-warning" title="Edit"><i class="fas fa-edit"></i></a><button type="button" class="btn btn-sm btn-danger btn-hapus" data-id="'+item.id_admin+'" title="Hapus"><i class="fas fa-trash"></i></button></div></td></tr>';});$('#dataAdmin').html(html);pagingAdmin($('#dataAdmin .baris-admin'),parseInt($('#jumlahAdmin').val(),10));}
	function muatAdmin(){$.ajax({url:'<?php echo site_url('admin/admin_user/list_data'); ?>',type:'POST',dataType:'json',success:function(result){semuaAdmin=result.data||[];tampilkanAdmin();},error:function(){$('#dataAdmin').html('<tr><td colspan="7" class="text-center text-danger">Data gagal dimuat.</td></tr>');}});}
