...
class Admin_base extends CI_Controller
{
	...
	protected function render_admin($view, $title, $active_menu, $data = array())
	{
		$data = array_merge($this->prepare_data($title, $active_menu), $data);
		$this->render($view, $data);
	}

	protected function rich_text_input($field)
	{
		return $this->sanitize_rich_text((string) $this->input->post($field, FALSE));
	}
	...
	protected function sanitize_rich_text($html)
	{
		...
		$html = preg_replace('/href\\s*=\\s*(?:(["\'])\\s*(?:javascript|data|vbscript):.*?\\1|(?:javascript|data|vbscript):[^\\s>]+)/i', 'href="#"', $html);

		return trim(strip_tags($html, '<p><br><strong><b><em><i><u><s><h1><h2><h3><h4><h5><h6><ol><ul><li><blockquote><pre><code><a><span>'));
	}
	...
	protected function ajax_save_response()
	{
		...
		if (!empty($this->crud['nullable_fields'])) {
			foreach ($this->crud['nullable_fields'] as $field) {
